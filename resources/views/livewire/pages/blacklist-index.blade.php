<div>
    <div class="mb-6 flex items-center justify-between">
        ...
        <button wire:click="$toggle('showForm')" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
        ...
        <form wire:submit="add" class="mb-6 bg-white rounded-lg shadow p-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-end">
                    <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Add to Blacklist</button>
                </div>
            </div>
        </form>
    @endif
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y">
                @forelse($blacklists as $blacklist)
                    <tr>
                        <td class="px-4 py-2 font-mono text-sm">{{ $blacklist->ip }}</td>
                        <td class="px-4 py-2 text-sm text-gray-600">{{ $blacklist->reason ?? '-' }}</td>
                        <td class="px-4 py-2 text-sm text-gray-500">{{ $blacklist->created_at->format('d M Y H:i') }}</td>
                        <td class="px-4 py-2 text-right">
                            <button wire:click="delete('{{ $blacklist->id }}')" class="text-red-600 hover:text-red-800 text-sm">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="px-4 py-3 text-sm text-gray-500 text-center">No blacklisted IPs.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <div class="mt-4">{{ $blacklists->links() }}</div>
</div>
